...
class Config:
    ...
    n_cpus: int = 1
    use_critic_fourier_features: Union[str, None] = None
...
env_name_ = ['FetchSlide-v1']#['FetchSlide-v1'] #,'FetchPickAndPlace-v1', 'FetchPush-v1', 'FetchReach-v1']
#n_workers_ = [2, 8, 16, 20]
#n_epochs_ = [50, 150, 200, 500]
#n_cpus_ = [1, 2, 4, 8]
n_workers_ = [20]#, 16 ,8 ,2]
n_epochs_ = [500]#, 200, 150, 50]
n_cpus_ = [8]#, 4, 2, 1]
use_critic_fourier_features_ = [True]#[True, False]
seed_ = [200]#, 200, 300, 400, 500]

exps = []
for (env_name, n_workers, n_epochs, n_cpus), seed, use_critic_fourier_features in product(zip(env_name_, n_workers_, n_epochs_, n_cpus_), seed_, use_critic_fourier_features_):
    exp = Config(env_name=env_name, n_workers=n_workers, n_epochs=n_epochs, n_cpus=n_cpus, seed=seed, use_critic_fourier_features=use_critic_fourier_features)
    exps.append(exp)
...
fetch = '/home/gridsan/bagrawalla/bvn2/bvn-master/fetch'
train_template = f'{fetch}/experiments/bvn/train_template_supercloud.py'
...
for idx, exp in enumerate(exps):
    ...
    with open(queue_file, 'w') as f:
        ...
        f.write(f'#SBATCH --cpus-per-task={30}\n')
        #f.write(f'#SBATCH --gres=gpu:1\n')
        f.write(f'#SBATCH --mem=30G\n')
        f.write(f'\n')
        ...
        f.write('python '+ f'experiments/bvn/train_{idx}.py\n')
